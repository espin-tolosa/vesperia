...
if(true){
    ...
    float VIEW_ROTZ = 0;
    int MAX_DEPTH = 2;
    unsigned NEL_Y = 100+1;
    unsigned NEL_X = 100+1; double DX = 0.1;
    unsigned NEL_Z = 100+1; double DZ = 0.1;
    int t=0;
    int tfin = 100;
...
//with SweepFace to create a 3D hexaedral structured-mesh
{
    ...
    if(Write_Vec3)
	{
	//#include "issues/WriteVec3.h" //This writes the Item cloud of points being fitted by the octree
	}
}//end of scope of 2D mesh-surface: Surface 

// LOOP PRINCIPAL
	Window window("samuel Engine v0.0", 1200,720);
	while (!window.closed())
	{
